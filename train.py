...
def train(epoch, trainDataloader, net, criterion, optimizer):
    net.train()
    epoch_loss = 0
    for iteration, batch in enumerate(trainDataloader, 0):
        ...
        if iteration % args.printEvery == 0:
            print("===> Epoch[{}]({}/{}): Avg. Loss: {:.4f}".format(epoch, iteration + 1, len(trainDataloader),
                                                                    epoch_loss / args.printEvery))
            epoch_loss = 0


def validate(valDataloader, net, criterion):
    net.eval()
    avg_mse = 0
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.nEpochs + 1):
        train(epoch, trainDataloader, net, criterion, optimizer)
        validate(valDataloader, net, criterion)
